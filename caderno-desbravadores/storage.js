...
export async function saveEvidenceFiles(files, key) {
  ...
  try {
    ...
    // Evita deixar arquivos órfãos no Storage quando um upload posterior
    // falhar antes de a submissão ser salva no banco.
    const uploadedPaths = saved.map((file) => file.path || file.id).filter(Boolean);
    if (uploadedPaths.length) {
      // Usa a mesma rota autorizada da exclusão individual, evitando
      // depender de permissões DELETE do Storage no navegador.
      await Promise.allSettled(
        uploadedPaths.map((uploadedPath) => deleteEvidenceFile(uploadedPath))
      );
    }
    throw error;
  }
