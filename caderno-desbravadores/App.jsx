          ...
          <>
            ...
            >
              ...
              <EvidenceGallery
                ...
                canDelete={editable && open && (submission.status === 'submitted' || submission.status === 'adminRejected')}
...
function ClassPage({
  ...
  const handleSubmit = async (item, data) => {
    ...
    if (old.status && !['none', 'adminRejected'].includes(old.status)) {
  ...
  const handleDeleteFile = async (item, file) => {
    ...
    if (!current?.files?.length || !['submitted', 'adminRejected'].includes(current.status)) return;
      ...
      const next = {
        ...
        submissions: {
          ...
          [key]: {
            ...
            // Depois que o próprio Desbravador remove uma evidência,
            // a entrega volta para rascunho para que ele possa corrigir
            // e reenviar sem precisar passar primeiro pela liderança.
            status: 'none',
            updatedAt: new Date().toISOString(),
            adminComment: '',
            regionalComment: ''
